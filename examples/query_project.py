...
from snyker import APIClient, GroupPydanticModel, IssuePydanticModel, ProjectPydanticModel
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Query a Snyk project and list its issues.")
    parser.add_argument("--project-id", required=True, help="The ID of the Snyk project to query.")
    parser.add_argument("--org-id", required=True, help="The ID of the Snyk organization the project belongs to.")
    ...
    try:
        ...
        if not organization:
            logger.error(f"Could not find organization with ID: {org_id} in group {group.name}")
            sys.exit(1)

        logger.info(f"Fetching project: {project_id}")
        project = organization.get_specific_project(project_id)
        if not project:
            logger.error(f"Could not find project with ID: {project_id} in organization {organization.name}")
            sys.exit(1)
        ...
        for issue in issues:
            ...
            print(f"    Issue Resolved At: {attrs.resolved_at.isoformat() if attrs.resolved_at else 'N/A'}")
            
            if attrs.problems:
                for problem in attrs.problems:
                    print(f"    Problem ({problem.source} - {problem.id}):")
                    print(f"      - Updated At: {problem.updated_at.isoformat() if problem.updated_at else 'N/A'}")
                    print(f"      - Disclosed At: {problem.disclosed_at.isoformat() if problem.disclosed_at else 'N/A'}")
                    print(f"      - Discovered At: {problem.discovered_at.isoformat() if problem.discovered_at else 'N/A'}")

            if attrs.severities:
                for severity in attrs.severities:
                    print(f"    Severity ({severity.source} - {severity.level}):")
                    print(f"      - Modification Time: {severity.modification_time.isoformat() if severity.modification_time else 'N/A'}")
