...
def export_group_policies_to_csv(api_client: APIClient, group_id: Optional[str] = None):
    ...
    try:
        logger.info(
            f"Attempting to get group instance for group_id: {group_id if group_id else 'auto-discover'}"
        )
        group = GroupPydanticModel.get_instance(
            api_client=api_client, group_id=group_id
        )
        ...
        organizations = (
            group.organizations
        )  # This will trigger fetch_organizations if not already loaded
        if not organizations:
            logger.warning(f"No organizations found for group '{group.name}'.")
            return

        logger.info(
            f"Found {len(organizations)} organizations in group '{group.name}'."
        )

        for org in organizations:
            logger.info(
                f"Fetching policies for organization: {org.name} (ID: {org.id})..."
            )
            try:
                ...
                logger.info(
                    f"Found {len(org_policies)} policies in organization {org.name}."
                )
                for policy in org_policies:
                    ...
                    if policy.conditions_group:
                        try:
                            ...
                            conditions_group_dict = policy.conditions_group.model_dump(
                                mode="json"
                            )
                            conditions_group_json = json.dumps(conditions_group_dict)
                        except Exception as e:
                            logger.error(
                                f"Error serializing conditions_group for policy {policy.id}: {e}"
                            )
                ...
                logger.error(
                    f"Error fetching policies for organization {org.name}: {e_org_policies}",
                    exc_info=True,
                )
        ...
        for org in organizations:
            logger.info(
                f"Fetching projects for organization: {org.name} (ID: {org.id}) to check for v1 ignores..."
            )
            ...
            logger.info(
                f"Found {len(projects)} projects in {org.name}. Now fetching v1 ignores for each."
            )
            for project in projects:
                try:
                    ...
                    logger.info(
                        f"Found {len(v1_ignores)} v1 ignores for project {project.name} ({project.id})."
                    )
                        ...
                        for issue_id, ignores in ignore_item.items():
                            for ignore_rule in ignores:
                                ...
                                policy_row = {
                                    ...
                                    "policy_ignore_type": ignore_details.get(
                                        "reasonType", "Unknown"
                                    ),
                                    "policy_expires": ignore_details.get("expires", ""),
                                    "policy_reason": ignore_details.get(
                                        "reason", "No reason provided"
                                    ),
                                    "policy_created_by_name": ignore_details.get(
                                        "ignoredBy", {}
                                    ).get("name", "Unknown"),
                                    "policy_conditions_group_json": json.dumps(
                                        {"v1_ignore_path": ignore_rule.get("path", "*")}
                                    ),
                                }
                                all_policies_data.append(policy_row)
                except Exception as e_v1_ignore:
                    logger.error(
                        f"Error fetching v1 ignores for project {project.name}: {e_v1_ignore}",
                        exc_info=True,
                    )
                    continue
        ...
        if not all_policies_data:
            logger.info(
                "No policies or v1 ignores found across all organizations in the group."
            )
            return
        ...
        csv_fieldnames = [
            ...
            "policy_expires",
            "policy_reason",
            "policy_created_by_name",
            "policy_conditions_group_json",
        ]

        logger.info(
            f"Writing {len(all_policies_data)} policies and ignores to CSV file: {csv_filename}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Export Snyk Group policies to a CSV file."
    )
    parser.add_argument(
        "--snyk-token",
        type=str,
    ...
    parser.add_argument(
        "--group-id",
        type=str,
        default=os.environ.get("SNYK_GROUP_ID", EXAMPLES_CONFIG.get("group_id")),
    ...
    if not args.snyk_token:
        logger.error(
            "Snyk token is required. Please provide it via --snyk-token argument or SNYK_TOKEN environment variable."
        )
    else:
