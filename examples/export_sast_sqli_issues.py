...
def main():
    ...
    try:
        ...
        logger.info(
            f"Fetched {len(sast_issues)} SAST issues. Analyzing for CWE-89 (SQL Injection)..."
        )
        ...
        if sast_sqli_issues:
            ...
            for issue in sast_sqli_issues:
                ...
                if (
                    issue.relationships
                    and issue.relationships.scan_item
                    and issue.relationships.scan_item.id
                ):
            ...
            logger.info(
                f"Found {len(sast_sqli_issues)} SAST SQL Injection (CWE-89) issues."
            )
        else:
            print(
