...
def recursive_find_value_path(
    data_structure: Any, search_value: str, current_path: str = ""
) -> Optional[str]:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Find a Snyk issue by a specific attribute value within a group."
    )
    parser.add_argument(
        "search_value",
        type=str,
        help="The attribute value to search for within issues.",
    )
    ...
    try:
        logger.info(
            f"Fetching all issues for Group '{group.name}' to search for value '{search_value}'..."
        )
        ...
        if found_issues_list:
            ...
            for idx, (issue_obj, path) in enumerate(found_issues_list):
                ...
                print(
                    f"  Issue Type: {issue_obj.attributes.type if issue_obj.attributes else 'N/A'}"
                )
                print(f"  Path to value: {path}")

                project_info = "N/A"
                if (
                    issue_obj.relationships
                    and issue_obj.relationships.scan_item
                    and issue_obj.relationships.scan_item.id
                ):
                ...
                logger.info(
                    f"Match {idx + 1}: Found value '{search_value}' in Issue ID {issue_obj.id} at path '{path}'."
                )
        else:
            print(
            ...
            logger.info(
                f'Value "{search_value}" not found in any of the {len(all_issues)} issues checked.'
            )
