...
class ProjectPydanticModel(BaseModel):
    ...
    def from_api_response(
        ...
        if fetch_full_details_if_summary and (
            "attributes" not in project_data or "relationships" not in project_data
        ):
            project_id_to_fetch = project_data.get("id")
            if not project_id_to_fetch:
                raise ValueError(
                    "Project data must contain an 'id' to fetch full details."
                )
            logger.debug(
                f"[Project ID: {project_id_to_fetch}] Summary data. Fetching full details..."
            )

            uri = f"/rest/orgs/{organization.id}/projects/{project_id_to_fetch}"
            headers = {
                "Content-Type": "application/json",
                "Authorization": f"token {api_client.token}",
            }
            ...
            try:
                ...
                logger.error(
                    f"Failed to fetch full details for project {project_id_to_fetch}: {e}"
                )
        ...
        instance._logger.info(
            f"[Project ID: {instance.id}] Created project object for '{instance.name}'"
        )
    ...
    def target_id(self) -> Optional[str]:
        ...
        if (
            self.relationships
            and self.relationships.target
            and self.relationships.target.data
        ):
    ...
    def _fetch_integration_details(self) -> None:
        ...
        if org_integrations:
            for integration_dict in org_integrations:
                ...
                if project_origin is None:
                    self._logger.debug(
                        f"[Project ID: {self.id}] Origin is None, cannot match integration."
                    )
                    continue
                ...
                if not match:  # Handle common variations
                    if (
                        project_origin_lower == "github-enterprise"
                        and integration_name_from_api == "github enterprise"
                    ):
                        match = True
                    elif (
                        project_origin_lower == "azure-repos"
                        and integration_name_from_api == "azure repos"
                    ):
                        match = True
                ...
                if match:
                    ...
                    self._logger.info(
                        f"[Project ID: {self.id}] Matched integration for origin '{self.origin}' with ID '{self._integration_id}' and repoUrl '{self._repo_url}'"
                    )
        ...
        if not found_integration:
            self._logger.warning(
                f"[Project ID: {self.id}] No matching integration found for origin '{self.origin}'."
            )
    ...
    def fetch_issues(
        ...
        self._logger.info(
            f"[Project ID: {self.id}] Fetched {len(self._issues)} issues."
        )
        return self._issues
    ...
    def test_api(self, continuous_monitor: bool = False) -> Optional[Dict[str, Any]]:
        ...
        if not self.integration_id or not self.repo_url:
            self._logger.error(
                f"[Project ID: {self.id}] Missing integrationId or repoUrl for testApi. Cannot proceed."
            )
            return None
        ...
        try:
            while True:
                try:
                    self._logger.debug(
                        f"Posting to testApi: {uri} with payload: {json.dumps(payload)}"
                    )
                    response_obj = requests.post(
                        uri, headers=headers, json=payload, timeout=30
                    )
                    response_obj.raise_for_status()
                    self._logger.debug(
                        f"testApi POST successful: {response_obj.status_code}"
                    )
                    break
                except requests.exceptions.HTTPError as err:
                    if err.response is not None and err.response.status_code == 429:
                        self._logger.debug(
                            f"Rate limit (429) on testApi POST. Retrying in {local_delay}s. Headers: {err.response.headers}"
                        )
                        time.sleep(local_delay)
                        local_delay = min(local_delay * 2, 60)
                    elif err.response is not None:
                        self._logger.error(
                            f"HTTP error on testApi POST: {err.response.status_code} - {err.response.text}",
                            exc_info=True,
                        )
                        return None
                    else:
                        self._logger.error(
                            f"Request error on testApi POST (no response object): {err}",
                            exc_info=True,
                        )
                        return None
                except requests.exceptions.RequestException as req_err:
                    self._logger.error(
                        f"Generic request error on testApi POST: {req_err}",
                        exc_info=True,
                    )
                    return None
            ...
            if response_obj and response_obj.status_code == 201:
                polling_uri = (
                    response_obj.json().get("links", {}).get("self", {}).get("href")
                )
                if not polling_uri:
                    self._logger.error(
                        "testApi POST successful, but no polling link ('links.self.href') found."
                    )
                    return None
                ...
                while True:
                    try:
                        poll_response = requests.get(
                            polling_uri, headers=headers, timeout=30
                        )
                        poll_response.raise_for_status()
                        poll_data = poll_response.json()
                        current_state = (
                            poll_data.get("data", {}).get("attributes", {}).get("state")
                        )
                        ...
                        if current_state == "in_progress":
                            if not in_progress_displayed_flag:
                                self._logger.info(
                                    f"SARIF generation in progress for {self.repo_url}..."
                                )
                            ...
                            self._logger.info(
                                f"SARIF generation completed for {self.repo_url}."
                            )
                            findings_url = (
                                poll_data.get("data", {})
                                .get("attributes", {})
                            ...
                            if not findings_url:
                                self._logger.error(
                                    "SARIF generation completed, but no findings_url found."
                                )
                                return None
                            ...
                            if sarif_response.content:
                                ...
                                self._logger.warning(
                                    "SARIF findings_url returned empty content."
                                )
                                return None
                        elif current_state in ["failed", "error"]:
                            self._logger.error(
                                f"SARIF generation failed with state: {current_state}. Data: {poll_data}"
                            )
                            return None
                        else:
                            self._logger.warning(
                                f"SARIF generation in unknown state: {current_state}. Data: {poll_data}"
                            )
                            time.sleep(local_delay)

                    except requests.exceptions.HTTPError as poll_err:
                        if (
                            poll_err.response is not None
                            and poll_err.response.status_code == 429
                        ):
                            self._logger.debug(
                                f"Rate limit (429) while polling testApi. Retrying in {local_delay}s."
                            )
                            time.sleep(local_delay)
                            local_delay = min(local_delay * 2, 60)
                        elif poll_err.response is not None:
                            self._logger.error(
                                f"HTTP error while polling testApi: {poll_err.response.status_code} - {poll_err.response.text}",
                                exc_info=True,
                            )
                            return None
                        else:
                            self._logger.error(
                                f"Request error while polling testApi (no response object): {poll_err}",
                                exc_info=True,
                            )
                            return None
                    except requests.exceptions.RequestException as req_err_poll:
                        self._logger.error(
                            f"Generic request error while polling testApi: {req_err_poll}",
                            exc_info=True,
                        )
                        return None
                    except Exception as e_poll:
                        self._logger.error(
                            f"Unexpected error during testApi polling: {e_poll}",
                            exc_info=True,
                        )
                        return None
            elif response_obj:
                self._logger.error(
                    f"Initial testApi POST call failed with status {response_obj.status_code}: {response_obj.text}"
                )

        except Exception as e_main:
            self._logger.error(
                f"An unexpected error occurred in test_api main try block: {e_main}",
                exc_info=True,
            )
        ...
        try:
            ...
            if self.project_type == "sast":
                if response_data:
                    ...
                    self._logger.debug(
                        f"SAST ignores for project {self.id}: {json.dumps(ignores, indent=2)}"
                    )
                else:
                    self._logger.info(
                        f"No SAST ignores found for project {self.id} or empty response."
                    )
            else:
                self._logger.debug(
                    f"Ignores for project {self.id} (type: {self.project_type}): {json.dumps(response_data, indent=2)}"
                )
                if isinstance(response_data, list):
                    ignores = response_data
                elif response_data:
                    self._logger.warning(
                        f"get_ignores_v1 for project type '{self.project_type}' received non-list data: {type(response_data)}. Assigning as is."
                    )
                    ignores = [response_data]
                else:
                    self._logger.info(
                        f"No ignores found for project {self.id} (type: {self.project_type})."
                    )

        except requests.exceptions.RequestException as e:
            self._logger.error(
                f"RequestException in get_ignores_v1 for project {self.id}, URL {uri}: {e}",
                exc_info=True,
            )
        except json.JSONDecodeError as e_json:
            self._logger.error(
                f"JSONDecodeError in get_ignores_v1 for project {self.id}: {e_json}",
                exc_info=True,
            )
        except Exception as e:
            self._logger.error(
                f"Unexpected error in get_ignores_v1 for project {self.id}: {e}",
                exc_info=True,
            )
...
ProjectPydanticModel.model_rebuild()
