        ...
        if isinstance(data, dict):
            ...
            if isinstance(source_location, dict):
                ...
                if isinstance(region, dict):
                    ...
                    data["start_line"] = (
                        start_info.get("line") if isinstance(start_info, dict) else None
                    )
                    data["start_column"] = (
                        start_info.get("column")
                        if isinstance(start_info, dict)
                        else None
                    )
                    data["end_line"] = (
                        end_info.get("line") if isinstance(end_info, dict) else None
                    )
                    data["end_column"] = (
                        end_info.get("column") if isinstance(end_info, dict) else None
                    )
        return data
...
class IssueAttributes(BaseModel):
    ...
    type: Optional[str] = (
        None  # Added field for issue type (e.g., "license", "vuln", "iac")
    )
...
class IssuePydanticModel(BaseModel):
    ...
    def _fetch_project_if_needed(self) -> None:
        ...
        if (
            self.relationships
            and self.relationships.scan_item
            and self.relationships.scan_item.type == "project"
        ):
            project_id = self.relationships.scan_item.id

        if (
            not org_for_project_fetch
            and self.relationships
            and self.relationships.organization
        ):
            org_id_from_rel = (
                self.relationships.organization.id
                if self.relationships.organization
                else None
            )
            if org_id_from_rel:
                self._logger.debug(
                    f"[Issue ID: {self.id}] Attempting to get Organization context (ID: {org_id_from_rel}) for project fetch."
                )
                if self._group:
                    org_for_project_fetch = self._group.get_organization_by_id(
                        org_id_from_rel
                    )
            else:
                self._logger.debug(
                    f"[Issue ID: {self.id}] No organization ID in relationships to fetch Organization context."
                )

        if project_id and org_for_project_fetch:
            self._logger.debug(
                f"[Issue ID: {self.id}] Lazily fetching Project (ID: {project_id}) via Org (ID: {org_for_project_fetch.id})."
            )
            try:
                project_model = org_for_project_fetch.get_specific_project(
                ...
                if project_model:
                    ...
                    self._logger.warning(
                        f"[Issue ID: {self.id}] Failed to fetch Project (ID: {project_id}). Project model was None."
                    )
            except Exception as e:
                self._logger.error(
                    f"[Issue ID: {self.id}] Error fetching Project (ID: {project_id}): {e}",
                    exc_info=True,
                )
        elif not project_id:
            self._logger.warning(
            ...
            self._logger.warning(
                f"[Issue ID: {self.id}] No Organization context to fetch Project (ID: {project_id})."
            )
