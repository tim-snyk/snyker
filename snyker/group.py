...
class GroupPydanticModel(BaseModel):
    ...
    def _fetch_group_data_by_id(
        ...
        try:
            ...
            logger.error(
                f"Error fetching group data for ID {group_id}: {e}", exc_info=True
            )
            raise ValueError(f"Failed to fetch data for group {group_id}.") from e

    @classmethod
    def _fetch_all_groups_data(
        cls, api_client: APIClient, params: Optional[Dict[str, Any]] = None
    ) -> List[Dict[str, Any]]:
    ...
    def get_instance(
        ...
        if group_id:
            ...
            if len(all_groups) == 1:
                ...
                group_names = [
                    g.get("attributes", {}).get("name", g.get("id", "Unknown"))
                    for g in all_groups
                ]
                logger.error(
        ...
        if (
            not group_data
            or "attributes" not in group_data
            or "name" not in group_data["attributes"]
        ):
            gid_for_log = group_id or "auto-discovered"
            logger.error(
                f"Failed to initialize group {gid_for_log}. Raw data incomplete or missing name: {group_data}"
            )
            raise ValueError(
                f"Failed to initialize group {gid_for_log}. Data incomplete."
            )
        ...
        instance._logger.info(
            f"[Group ID: {instance.id}] Created group object for '{instance.name}'"
        )
    ...
    def fetch_organizations(
        ...
        try:
            ...
            self._logger.error(
                f"[Group ID: {self.id}] Error paginating organizations: {e_paginate}",
                exc_info=True,
            )
            self._organizations = []
            return self._organizations
    ...
    def fetch_issues(
        ...
        try:
            ...
            self._logger.error(
                f"[Group ID: {self.id}] Error paginating group issues: {e_paginate}",
                exc_info=True,
            )
            if not params:
                self._issues = []
        ...
        for future in concurrent.futures.as_completed(issue_futures):
            try:
                ...
                self._logger.error(
                    f"[Group ID: {self.id}] Error instantiating Issue model: {e_future}",
                    exc_info=True,
                )
        ...
        self._logger.info(
            f"[Group ID: {self.id}] Fetched and instantiated {len(issue_results)} group issues with params: {json.dumps(_params)}."
        )
    ...
    def get_specific_asset(
        ...
        """
        self._logger.debug(
            f"[Group ID: {self.id}] Fetching specific asset by ID: {asset_id}..."
        )
        _params = params if params is not None else {}
        ...
        try:
            response = self._api_client.get(
                uri, headers=headers, params=request_api_params
            )
            asset_data = response.json().get("data", {})
            if asset_data:
                return Asset.from_api_response(
                    asset_data, api_client=self._api_client, group=self
                )
            else:
                self._logger.warning(
                    f"[Group ID: {self.id}] No data found for asset {asset_id}."
                )
                return None
        except Exception as e:
            self._logger.error(
                f"[Group ID: {self.id}] Error fetching asset {asset_id}: {e}",
                exc_info=True,
            )
            return None

    def get_assets_by_query(
        self, query: Dict[str, Any], params: Optional[Dict[str, Any]] = None
    ) -> List[Asset]:
        ...
        """
        self._logger.debug(
            f"[Group ID: {self.id}] Fetching assets by query: {json.dumps(query)}..."
        )
        _params = params if params is not None else {}
        ...
        if not query:
            raise ValueError(
                "Query parameter (dict) is required for get_assets_by_query."
            )

        asset_data_items: List[Dict[str, Any]] = []
        try:
            response_obj = self._api_client.post(
                uri, headers=headers, params=request_api_params, data=query
            )
            ...
            while True:
                ...
                if next_page_link:
                    self._logger.debug(
                        f"Fetching next page of assets from POST search: {next_page_link}"
                    )
            ...
            self._logger.error(
                f"[Group ID: {self.id}] Error during asset query: {e}", exc_info=True
            )
            return []
        ...
        for asset_data in asset_data_items:
            future = self._api_client.submit_task(
                Asset.from_api_response, asset_data, self._api_client, self
            )
        ...
        for future in concurrent.futures.as_completed(asset_futures):
            try:
                ...
                self._logger.error(
                    f"[Group ID: {self.id}] Error instantiating Asset model from query: {e_future}",
                    exc_info=True,
                )

        self._logger.info(
            f"[Group ID: {self.id}] Found and instantiated {len(asset_results)} assets from query."
        )
        return asset_results

    def get_organization_by_id(
        self, org_id: str
    ) -> Optional[OrganizationPydanticModel]:
        ...
        self._logger.debug(
            f"[Group ID: {self.id}] Organization {org_id} not in cache, fetching directly."
        )
        from .organization import OrganizationPydanticModel  # Local import
        ...
        try:
            ...
            if org_data:
                ...
                if org_group_id != self.id:
                    self._logger.warning(
                        f"Org {org_id} fetched but belongs to group {org_group_id}, not {self.id}."
                    )
                return OrganizationPydanticModel.from_api_response(
                    org_data, self._api_client, self
                )
            return None
        except Exception as e:
            self._logger.error(
                f"[Group ID: {self.id}] Error fetching organization {org_id}: {e}",
                exc_info=True,
            )
            return None


GroupPydanticModel.model_rebuild()
