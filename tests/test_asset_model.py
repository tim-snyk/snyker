...
class TestAssetPydanticModelIntegration(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if cls.group:
            ...
            assets = cls.group.get_assets_by_query(
                query=asset_query, params={"limit": 10}
            )
            if assets:
                cls.asset_to_test = assets[0]  # Take the first one for testing
                cls.api_client.logger.info(
                    f"Using asset {cls.asset_to_test.id} ({cls.asset_to_test.name}) for tests."
                )
    ...
    def test_asset_instantiation(self):
        ...
        self.assertIn(
            KNOWN_ASSET_NAME_CONTAINS.lower(), self.asset_to_test.name.lower()
        )
    ...
    def test_lazy_load_asset_organizations(self):
        ...
        if organizations:
            ...
            self.api_client.logger.warning(
                f"No organizations found for asset {self.asset_to_test.id} during lazy load."
            )
    ...
    def test_lazy_load_asset_projects(self):
        ...
        if projects:
            ...
            self.api_client.logger.warning(
                f"No projects found for asset {self.asset_to_test.id} during lazy load."
            )
