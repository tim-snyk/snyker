...
if __name__ == "__main__":
    ...
    snyk_cli.flight_check(
        minimum_version="1.1295.4"
    )  # Check Snyk CLI presence and version

    snyk_cli.change_directory(
        "/Users/timgowan/git/snyker"
    )  # Change to your git repo directory
    # Get the orgId from the asset
    repository_url = "https://github.com/tim-snyk/vulnado"
    assets = snyk_cli.find_assets_from_repository_url(
        repository_url
    )  # Find the asset(s) from the repository URL
    if len(assets) == 1:
        asset = assets[0]  # Assign it to singular Asset object
    org_id = snyk_cli.find_org_id_from_asset(
        asset
    )  # Find the org_id from the Asset object
    ...
    playbook = [
        ...
        },
        {
            ...
            "--project-business-criticality": snyk_cli.get_business_criticality_from_asset(
                asset
            ),
    ...
    snyk_cli.change_directory(
        "/Users/timgowan/git/juice-shop"
    )  # Change to a specific git repo directory
